    ...
    for i, body in enumerate(data["floatBody"]):
        ...
        for etype, indices in zip(el_types, el_indices):
            ...
            if etype == "net":
                ...
                print(f"  Unknown element type: {etype}, defaulting Sn to 0.")
            ...
            print(f"  Set Sn={val} for {count} nodes (type '{etype}', indices {start_idx_1based}-{end_idx_1based})")

        # Add to body
        body["attachNodeSn"] = attach_node_sn
    
    # Save new JSON
    output_path = str(json_path).replace(".json", "_withSn.json")
...
if __name__ == "__main__":
    ...
    if not json_file.exists():
        ...
        # Try absolute path from user info
